CLASS zsheep_cl_http_handler DEFINITION
  PUBLIC
  CREATE PROTECTED.
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS factory_cloud
      ...
      RETURNING
        VALUE(result) TYPE REF TO zsheep_cl_http_handler.
    ...
    METHODS main
      IMPORTING
        s_config TYPE zsheep_if_types=>ty_s_http_config OPTIONAL.

  PROTECTED SECTION.

    CLASS-DATA so_sticky_handler TYPE REF TO zsheep_cl_core_http_post.
    DATA mo_server TYPE REF TO zsheep_cl_abap_api_http.
    DATA ms_session_attributes TYPE zsheep_if_types=>ty_s_http_handler_attributes.
    ...
    TYPES:
      BEGIN OF ty_s_http_res,
        ...
        t_header      TYPE zsheep_if_types=>ty_t_name_value,
      END OF ty_s_http_res.

    DATA ms_req TYPE ty_s_http_req.
    DATA ms_res TYPE ty_s_http_res.
    DATA ms_config TYPE zsheep_if_types=>ty_s_http_config.

    METHODS set_config
      IMPORTING
        is_custom_config TYPE zsheep_if_types=>ty_s_http_config.
    ...
    METHODS session_handling
      IMPORTING
        attributes TYPE zsheep_if_types=>ty_s_http_handler_attributes.
...
CLASS zsheep_cl_http_handler IMPLEMENTATION.
...
*      ms_req_config-src     = `https://sdk.openui5.org/nightly/2/resources/sap-ui-core.js`.
  ...
  METHOD factory.
    ...
    result = NEW #( ).
    result->mo_server = zsheep_cl_abap_api_http=>factory( server ).
  ...
  METHOD factory_cloud.
    ...
    result = NEW #( ).
    result->mo_server = zsheep_cl_abap_api_http=>factory_cloud(
        req = req
        res = res ).
  ...
  METHOD http_post.
    ...
    IF so_sticky_handler IS NOT BOUND.
      DATA(lo_post) = NEW zsheep_cl_core_http_post( ms_req-body ).
    ELSE.
    ...
    TRY.
        IF lo_post IS BOUND.
          DATA(li_app) = CAST zsheep_if_app( lo_post->mo_action->mo_app->mo_app ).
  ...
  METHOD get_index_html.
    ...
    IF ms_config-styles_css IS INITIAL.
      DATA(lv_style_css) = zsheep_cl_app_style_css=>get( ).
    ELSE.
    ...
    result = `<!DOCTYPE html>` && |\n| &&
             ...
             `      "z2ui5/manifest.json": '` && zsheep_cl_app_manifest_json=>get( ) && `',` && |\n| &&
             `      "z2ui5/Component.js": function(){` &&  zsheep_cl_app_component_js=>get( ) && ms_config-custom_js && `},` && |\n| &&
             `      "z2ui5/css/style.css": '` && lv_style_css && `',` && |\n| &&
             `      "z2ui5/model/models.js": function(){` &&  zsheep_cl_app_models_js=>get( ) && `},` && |\n| &&
             `      "z2ui5/view/App.view.xml": '` && zsheep_cl_app_app_xml=>get( ) && `',` && |\n| &&
             `      "z2ui5/controller/App.controller.js": function(){` && zsheep_cl_app_app_js=>get( ) && `},` && |\n| &&
             `      "z2ui5/view/View1.view.xml": '` && zsheep_cl_app_view1_xml=>get( )  && `',` && |\n| &&
             `      "z2ui5/controller/View1.controller.js": function(){` && zsheep_cl_app_view1_js=>get( ) && `},` && |\n| &&
             `      "z2ui5/cc/Server.js": function(){` && zsheep_cl_app_server_js=>get( )    && `},` && |\n| &&
             `      "z2ui5/cc/DebugTool.fragment.xml": '` && zsheep_cl_app_debugtool_xml=>get( )   && `',` && |\n| &&
             `      "z2ui5/cc/DebugTool.js": function(){` && zsheep_cl_app_debugtool_js=>get( )  && `},` && |\n| &&
             `    });` && |\n| &&
