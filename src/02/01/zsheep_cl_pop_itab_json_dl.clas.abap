CLASS zsheep_cl_pop_itab_json_dl DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS factory
      ...
      RETURNING
        VALUE(r_result)       TYPE REF TO zsheep_cl_pop_itab_json_dl.
...
CLASS zsheep_cl_pop_itab_json_dl IMPLEMENTATION.


  METHOD factory.

    r_result = NEW #( ).
    r_result->mr_itab = zsheep_cl_util=>conv_copy_ref_data( itab ).
  ...
  METHOD zsheep_if_app~main.
    ...
    TRY.
        ...
        IF zsheep_cl_util=>rtti_check_class_exists( `z2ui5_dbt_cl_app_03` ) = abap_false.
          ...
          DATA(lx) = NEW zsheep_cx_util_error( val = lv_text ).
          client->nav_app_leave( zsheep_cl_pop_error=>factory( lx ) ).
      ...
      CATCH cx_root INTO DATA(x).
        client->nav_app_leave( zsheep_cl_pop_to_inform=>factory( x->get_text( ) ) ).
    ENDTRY.
  ENDMETHOD.
ENDCLASS.
