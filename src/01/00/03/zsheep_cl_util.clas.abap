CLASS zsheep_cl_util DEFINITION
  PUBLIC
  CREATE PUBLIC
  INHERITING FROM zsheep_cl_abap_api.
...
*  license: MIT.
    ...
    CLASS-METHODS rtti_get_t_ddic_fixed_values
      ...
      RETURNING
        VALUE(result) TYPE zsheep_cl_abap_api=>ty_t_fix_val ##NEEDED.
    ...
    CLASS-METHODS filter_get_sql_where
      IMPORTING
        val           TYPE  zsheep_cl_util=>ty_t_filter_multi
      RETURNING
        VALUE(result) TYPE string.
...
CLASS zsheep_cl_util IMPLEMENTATION.


  METHOD db_delete_by_handle.

    DELETE FROM zsheep_t_02
        WHERE
           uname = @uname
            AND handle = @handle
            AND handle2 = @handle2
  ...
  METHOD db_load_by_handle.
    ...
    DATA lt_db TYPE STANDARD TABLE OF zsheep_t_02 WITH EMPTY KEY.
  ...
  METHOD filter_update_tokens.
    ...
    DATA(lt_range) = zsheep_cl_util=>filter_get_range_t_by_token_t( result[ name = name ]-t_token ).
  ...
  METHOD json_parse.
    TRY.
        ...
        zsheep_cl_ajson=>parse( val )->to_abap(
          EXPORTING
  ...
  METHOD json_stringify.
    TRY.
        ...
        DATA(li_ajson) = CAST zsheep_if_ajson( zsheep_cl_ajson=>create_empty( ) ).
...
`x, nsis, objectivec, ocaml, pascal, perl, pgsql, php, plain_text, powershell, praat, prolog, properties, protobuf, python, r, razor, rdoc, rhtml, rst, ruby, rust, sass, scad, scala, scheme, scss, sh, sjs, smarty, snippets, soy_template, space, sql,` &&
  ...
  METHOD x_check_raise.
    ...
    IF when = abap_true.
      RAISE EXCEPTION TYPE zsheep_cx_util_error EXPORTING val = v.
    ENDIF.
