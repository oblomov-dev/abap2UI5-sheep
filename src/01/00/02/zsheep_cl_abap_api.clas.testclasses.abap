...
CLASS ltcl_test IMPLEMENTATION.
  ...
  METHOD test_func_get_uuid_32.
    ...
    DATA(lv_uuid) = zsheep_cl_abap_api=>uuid_get_c32( ).
  ...
  METHOD test_func_get_uuid_22.
    ...
    DATA(lv_uuid) = zsheep_cl_abap_api=>uuid_get_c22( ).
    cl_abap_unit_assert=>assert_not_initial( lv_uuid ).
    cl_abap_unit_assert=>assert_equals(
        act = 22
        exp = strlen( lv_uuid ) ).
  ...
  METHOD test_encoding.
    ...
    DATA(lv_xstring)  = zsheep_cl_abap_api=>conv_get_xstring_by_string( lv_string ).
    DATA(lv_string2)  = zsheep_cl_abap_api=>conv_encode_x_base64( lv_xstring ).
    DATA(lv_xstring2) = zsheep_cl_abap_api=>conv_decode_x_base64( lv_string2 ).
    DATA(lv_string3)  = zsheep_cl_abap_api=>conv_get_string_by_xstring( lv_xstring2 ).
  ...
  METHOD test_element_text.
    ...
    DATA(ls_result) = zsheep_cl_abap_api=>rtti_get_data_element_texts( `UNAME` ).
  ...
  METHOD test_classes_impl_intf.
    ...
    DATA(mt_classes) = zsheep_cl_abap_api=>rtti_get_classes_impl_intf( `IF_SERIALIZABLE_OBJECT`  ).
