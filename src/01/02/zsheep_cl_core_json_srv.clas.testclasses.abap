...
CLASS ltcl_test IMPLEMENTATION.
  ...
  METHOD request_json_to_abap.
    ...
    DATA(lo_mapper) = NEW zsheep_cl_core_json_srv( ).
    DATA(ls_result) = lo_mapper->request_json_to_abap( lv_payload ).

    DATA(ls_exp) = VALUE zsheep_if_core_types=>ty_s_http_request_post(
        s_front = VALUE #(
        id       = `ID_NR`
        view     = `MAIN`
        origin   = `ORIGIN`
    ...
    DATA(lt_tree) = VALUE zsheep_if_ajson_types=>ty_nodes_ts( ).
