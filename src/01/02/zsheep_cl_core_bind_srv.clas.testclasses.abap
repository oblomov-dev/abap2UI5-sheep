...
CLASS zsheep_cl_core_bind_srv DEFINITION LOCAL FRIENDS ltcl_test_bind.
...
CLASS ltcl_test_bind IMPLEMENTATION.
  ...
  METHOD test_one_way_w_x_error.
    ...
    DATA(lo_app) = NEW zsheep_cl_core_app( ).
    lo_app->mo_app = lo_app_client.

    DATA(lo_bind)  = NEW zsheep_cl_core_bind_srv( lo_app ).
