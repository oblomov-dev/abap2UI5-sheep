CLASS zsheep_cl_core_http_post DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    DATA ms_request      TYPE zsheep_if_core_types=>ty_s_http_request_post.
    DATA ms_response     TYPE zsheep_if_core_types=>ty_s_http_response_post.
...
CLASS zsheep_cl_core_http_post IMPLEMENTATION.


  METHOD constructor.

    mv_request_json = val.
    mo_action = NEW zsheep_cl_core_action( me ).
  ...
  METHOD main_begin.
    TRY.
        ...
        DATA(lo_json_mapper) = NEW zsheep_cl_core_json_srv( ).
        ...
        ELSEIF ms_request-s_control-app_start IS NOT INITIAL.
          NEW zsheep_cl_core_draft_srv( )->cleanup( ).
  ...
  METHOD main_end.
    ...
    ms_response = VALUE #(
        ...
        s_front-app    = zsheep_cl_util=>rtti_get_classname_by_ref( mo_action->mo_app->mo_app )
        ).
    ...
    IF CAST zsheep_if_app( mo_action->mo_app->mo_app )->check_sticky = abap_false.
  ...
  METHOD main_process.
    TRY.
        DATA li_client TYPE REF TO zsheep_cl_core_client.
        TRY.
            CREATE OBJECT li_client TYPE ('ZCL_2UI5_CUSTOM_CLIENT')
                EXPORTING
                    action = mo_action.
          CATCH cx_root.
            li_client = NEW zsheep_cl_core_client( mo_action ).
        ENDTRY.
        DATA(li_app)    = CAST zsheep_if_app( mo_action->mo_app->mo_app ).
