CLASS zsheep_cl_core_app_startup DEFINITION
  PUBLIC
  FINAL
  CREATE PROTECTED .
  ...
  PUBLIC SECTION.
    ...
    DATA client TYPE REF TO zsheep_if_client.
    DATA mv_check_initialized TYPE abap_bool.

    CLASS-METHODS factory
      RETURNING
        VALUE(result) TYPE REF TO zsheep_cl_core_app_startup.
  ...
  PROTECTED SECTION.
    DATA mt_classes TYPE zsheep_cl_util=>ty_t_classes.
...
CLASS zsheep_cl_core_app_startup IMPLEMENTATION.
  ...
  METHOD on_event_check.
    DATA li_app_test TYPE REF TO zsheep_if_app.

    TRY.
        ms_home-classname = zsheep_cl_util=>c_trim_upper( ms_home-classname ).
        ...
        ms_home-url = zsheep_cl_core_util_srv=>app_get_url(
                   client    = client
  ...
  METHOD view_display_start.
    ...
    DATA(page2) = zsheep_cl_xml_view=>factory( )->shell( )->page(
    ...
    DATA(lv_url_samples2) = zsheep_cl_core_util_srv=>app_get_url(
                  client    = client
                  classname = 'z2ui5_cl_demo_app_000' ).

    simple_form2->toolbar( )->title( `What's next?` ).

    IF zsheep_cl_util=>rtti_check_class_exists( `z2ui5_cl_demo_app_000` ).
  ...
  METHOD zsheep_if_app~main.
    ...
    IF client->get( )-check_on_navigated = abap_true.
      TRY.
          DATA(lo_f4) = CAST zsheep_cl_pop_to_select( client->get_app( client->get( )-s_draft-id_prev_app ) ).
  ...
  METHOD view_display_popup.
    ...
    DATA(page2) = zsheep_cl_xml_view=>factory_popup(
         )->dialog(
    ...
    simple_form2->checkbox( enabled = abap_false selected = zsheep_cl_util=>context_check_abap_cloud( ) ).

    DATA(lv_count) = CONV string( NEW zsheep_cl_core_draft_srv( )->count_entries( ) ).
    simple_form2->toolbar( )->title( `abap2UI5` ).
    simple_form2->label( `Version ` ).
    simple_form2->text( zsheep_if_app=>version ).
  ...
  METHOD z2ui5_on_event.
    ...
    DATA li_app TYPE REF TO zsheep_if_app.
...
*        client->nav_app_call( z2ui5_cl_core_app_info=>factory( ) ).
      ...
      WHEN 'VALUE_HELP'.
        TRY.
            mt_classes = zsheep_cl_util=>rtti_get_classes_impl_intf( zsheep_cl_util=>rtti_get_intfname_by_ref( li_app ) ).
          CATCH cx_root.
            client->message_box_display( `Unfortunately the value help is not available on your ABAP release!` ).
            RETURN.
        ENDTRY.
        client->nav_app_call( zsheep_cl_pop_to_select=>factory( mt_classes ) ).
    ENDCASE.
  ...
  METHOD z2ui5_on_init.
    ...
    ms_home-class_editable = abap_true.
    ms_home-btn_icon       = `sap-icon://validate`.
    ms_home-classname      = zsheep_cl_util=>rtti_get_classname_by_ref( NEW zsheep_cl_hello_world( ) ).
