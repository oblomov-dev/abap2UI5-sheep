CLASS zsheep_cl_core_app DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    DATA ms_draft   TYPE zsheep_if_types=>ty_s_get-s_draft.
    ...
    METHODS model_json_parse
      IMPORTING
        !iv_view  TYPE clike
        !io_model TYPE REF TO zsheep_if_ajson.
    ...
    CLASS-METHODS all_xml_parse
      IMPORTING
        !xml          TYPE clike
      RETURNING
        VALUE(result) TYPE REF TO zsheep_cl_core_app.
...
CLASS zsheep_cl_core_app IMPLEMENTATION.
  ...
  METHOD all_xml_stringify.
    ...
    TRY.
        ...
        result = zsheep_cl_util=>xml_stringify( me ).
      ...
      CATCH cx_root INTO DATA(x2).
        TRY.
            ...
            DATA(lo_dissolver) = NEW zsheep_cl_core_diss_srv(
              attri = mt_attri
              app   = mo_app ).

            lo_dissolver->main( ).
            lo_dissolver->main( ).
            lo_model = NEW zsheep_cl_core_attri_srv(
              attri = mt_attri
              app   = mo_app ).
            lo_model->attri_before_save( ).
  ...
  METHOD db_load.
    ...
    DATA(lo_db) = NEW zsheep_cl_core_draft_srv( ).
  ...
  METHOD db_save.
    ...
    IF mo_app IS BOUND.
      CAST zsheep_if_app( mo_app )->id_draft = ms_draft-id.
    ENDIF.
